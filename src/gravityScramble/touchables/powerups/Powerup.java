...
public class Powerup extends Touchable {
	...
	public void paintComponent(Graphics g, int x, int y) {
		...
		g.fillRect(drawPosX, 630 - altitude + y, 100, 10);
		g.fillRect(drawPosX, 440 - altitude + y, 10, 200);
		g.fillRect(drawPosX, 440 - altitude + y, 100, 10);
		g.fillRect(drawPosX + 90, 440 - altitude + y, 10, 200);
		...
		if (activating) {
			...
			g.fillRect(drawPosX, 440 - altitude + y, 100, 200);
			
			g.setColor(color);
			g.fillRect(drawPosX, 630 - altitude + y - counter, 100, 10);
		}
	}

	public void activate(Player player) {
		if ((isTouching(player) || counter > 0) && !player.getPowerUp().equals("fire")) {
			activating = true;
